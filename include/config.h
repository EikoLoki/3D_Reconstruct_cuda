...
static Mat lMatrix = (Mat_<double>(3, 3) << camera_fx_left,  camera_s_left, camera_cx_left,
                                                  0, camera_fy_left, camera_cy_left,
                                                  0,              0,              1);

static Mat rMatrix = (Mat_<double>(3, 3) << camera_fx_right,  camera_s_right, camera_cx_right,
                                                   0, camera_fy_right, camera_cy_right,
                                                   0,               0,               1);

static Mat lMatrixx2 = (Mat_<double>(3, 3) << camera_fx_left*2,  camera_s_left*2, camera_cx_left*2,
                                                      0, camera_fy_left*2, camera_cy_left*2,
                                                      0,                0,                1);

static Mat rMatrixx2 = (Mat_<double>(3, 3) <<  camera_fx_right*2,  camera_s_right*2, camera_cx_right*2,
                                                        0, camera_fy_right*2, camera_cy_right*2,
                                                        0,                 0,                 1);

static Mat distCoeffL = (Mat_<double>(5, 1) << RD_left[0], RD_left[1], TD_left[0], TD_left[1], RD_left[2]);
static Mat distCoeffR = (Mat_<double>(5, 1) << RD_right[0], RD_right[1], TD_right[0], TD_right[1], RD_right[2]);

static Mat R = (Mat_<double>(3,3) << 0.999715113352754, -0.006517133674422,-0.022961252203662 ,
...
static Mat T = (Mat_<double>(3,1) << -5.403825604029611, 0.133984745864434, -0.098250420949267);       // translation of right camera w.r.t left camera
...
public:
    ~Config();
    void getParameterFile(const std::string& filename);
    template <typename T>
