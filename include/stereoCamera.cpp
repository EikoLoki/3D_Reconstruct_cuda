...
stereoCamera::stereoCamera(){
    camera_fx_left = Config::get<float>("camera.lfx");
    camera_fy_left = Config::get<float>("camera.lfy");
    camera_cx_left = Config::get<float>("camera.lcx");
    camera_cy_left = Config::get<float>("camera.lcy");
    camera_s_left = Config::get<float>("camera.ls");
    camera_rd_left = Config::get<cv::Mat>("camera.lrd");
    camera_td_left = Config::get<cv::Mat>("camera.ltd");

    camera_fx_right = Config::get<float>("camera.rfx");
    camera_fy_right = Config::get<float>("camera.rfy");
    camera_cx_right = Config::get<float>("camera.rcx");
    camera_cy_right = Config::get<float>("camera.rcy");
    camera_s_right = Config::get<float>("camera.rs");
    camera_rd_right = Config::get<cv::Mat>("camera.rrd");
    camera_td_right = Config::get<cv::Mat>("camera.rtd");

    camera_baseline = Config::get<float>("camera.bl");
...
cv::Mat stereoCamera::getIntrinsic(stereoCamera::device_num num){
    switch(num){
    case left:{
        cv::Mat Intrinsic = (cv::Mat_<float>(3,3) << camera_fx_left, camera_s_left, camera_cx_left,
                                                                  0, camera_fy_left, camera_cy_left,
                                                                  0,              0,              1);
        return Intrinsic;
    }
    case right:{
        cv::Mat Intrinsic = (cv::Mat_<float>(3,3) << camera_fx_right, camera_s_right, camera_cx_right,
...
cv::Mat stereoCamera::getDistCoeff(stereoCamera::device_num num){
    switch(num){
    case left:{
        cv::Mat distCoeff = (cv::Mat_<float>(5,1) << camera_rd_left.at<float>(0), camera_rd_left.at<float>(1),
                                                     camera_td_left.at<float>(0), camera_td_left.at<float>(1),
                                                     camera_rd_left.at<float>(2));
        return distCoeff;
    }
    case right:{
        cv::Mat distCoeff = (cv::Mat_<float>(5,1) << camera_rd_right.at<float>(0), camera_rd_right.at<float>(1),
